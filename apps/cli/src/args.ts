/**
 * Commander adapter for the `kalix` command line.
 *
...
const HELP_EXAMPLES = `
Examples:
  kalix --profile web                          boot the web profile (same as: kalix web)
  kalix --profile headless "run the tests"     answer one task, print the result, and exit
  kalix --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  kalix --profile tui --resume <session>       arguments after the launcher flags reach the app
  kalix --profile web --help                   the web app's own flags and help
  kalix plugin --profile tui add <package>     install a plugin into the tui profile
`
...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  program
    ...
    .description('Kalix Code: boot a profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    ...
    .argument('[args...]', 'arguments for the booted profile\'s app (see: kalix --profile <name> --help)')
    .option('--profile <name>', 'the profile under $KALIX_HOME/profiles to boot')
  ...
  const web = program.command('web').description('boot the Kalix web profile (alias of --profile web); the web app\'s own flags follow')
  web
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: kalix web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
