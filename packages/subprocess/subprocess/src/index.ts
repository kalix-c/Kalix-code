...
import { DSH_ENV_PREFIX, KALIX_ENV_PREFIX } from './types.ts'
import type { SubprocessHandle, SubprocessSpawnSpec } from './types.ts'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from './types.ts'

export { DSH_ENV_PREFIX, KALIX_ENV_PREFIX } from './types.ts'
export type {
  CollectedOutput,
  DshEnvironment,
  DshEnvironmentKey,
 ...
 * `DSH_*`/`KALIX_*` names — the canonical base every Kalix child starts from. `PATH`,
 ...
 * credential or current managed fact goes through the spec's explicit `env`,
 * which merges after this scrub). Both scrubs match case-insensitively:
 * Windows environment names are case-insensitive, so a parent managed entry
 * would otherwise survive in the child; deliberate lowercase compatibility
 * names on POSIX are implausible. Exported as a plain function so spawners
...
export function scrubbedParentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (value !== undefined && !SENSITIVE_ENV_PATTERN.test(key)
      && !upperKey.startsWith(DSH_ENV_PREFIX) && !upperKey.startsWith(KALIX_ENV_PREFIX)) env[key] = value
  }
  return env
}
